...
// Account Returns View - Weekly absolute and percentage returns
export const viewAccountReturns = pgView('view_account_returns', {
  userId: integer('user_id'),
  accountKey: integer('account_key'),
  weekStart: date('week_start'),
  cumulativeTransfers: decimal('cumulative_transfers', { precision: 18, scale: 8 }),
  cumulativePortfolioValue: decimal('cumulative_portfolio_value', { precision: 18, scale: 8 }),
  weeklyTransfers: decimal('weekly_transfers', { precision: 18, scale: 8 }),
  absoluteReturn: decimal('absolute_return', { precision: 18, scale: 8 }),
  percentReturn: decimal('percent_return', { precision: 18, scale: 4 }),
  previousPortfolioValue: decimal('previous_portfolio_value', { precision: 18, scale: 8 })
}).as(sql`
  ...
  FROM weekly_returns
  WHERE 
    previous_portfolio_value IS NOT NULL AND 
    previous_portfolio_value != 0  -- Skip first week (no previous value)
...
export type ViewAccountReturns = typeof viewAccountReturns.$inferSelect;
