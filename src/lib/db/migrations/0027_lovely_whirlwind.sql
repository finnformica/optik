CREATE OR REPLACE VIEW "public"."monthly_performance" AS (
      ...
      -- Fallback for periods with no data: use total performance since inception
      fallback_data AS (
        SELECT 
          user_id,
          DATE_TRUNC('month', MIN(week_start::date)) as period_start,
          -- Total P&L since inception
          MAX(cumulative_portfolio_value::numeric) - MIN(cumulative_portfolio_value::numeric) - 
          (MAX(cumulative_transfers::numeric) - MIN(cumulative_transfers::numeric)) as period_pnl,
          -- Total percentage since inception
          CASE 
            WHEN MIN(cumulative_portfolio_value::numeric) > 0
            THEN ((MAX(cumulative_portfolio_value::numeric) - MIN(cumulative_portfolio_value::numeric)) - 
                  (MAX(cumulative_transfers::numeric) - MIN(cumulative_transfers::numeric))) * 100.0 / MIN(cumulative_portfolio_value::numeric)
            ELSE 0
          END as period_pnl_percent,
          MAX(cumulative_portfolio_value::numeric) as portfolio_value,
          MAX(cumulative_transfers::numeric) - MIN(cumulative_transfers::numeric) as period_transfers
        FROM account_value_over_time
        GROUP BY user_id
        HAVING COUNT(DISTINCT DATE_TRUNC('month', week_start::date)) = 1  -- Only one period exists
      )
      SELECT 
        user_id,
        period_start,
      ...
      SELECT 
        ...
        period_transfers
      FROM fallback_data
      WHERE user_id NOT IN (SELECT user_id FROM period_results)
...
CREATE OR REPLACE VIEW "public"."weekly_performance" AS (
      ...
      fallback_data AS (
        SELECT 
          user_id,
          DATE_TRUNC('week', MIN(week_start::date)) as period_start,
          -- Total P&L since inception
          MAX(cumulative_portfolio_value::numeric) - MIN(cumulative_portfolio_value::numeric) - 
          (MAX(cumulative_transfers::numeric) - MIN(cumulative_transfers::numeric)) as period_pnl,
          -- Total percentage since inception
        ...
        FROM account_value_over_time
        GROUP BY user_id
        HAVING COUNT(DISTINCT DATE_TRUNC('week', week_start::date)) = 1  -- Only one period exists
      )
      SELECT 
        user_id,
        period_start,
...
CREATE OR REPLACE VIEW "public"."yearly_performance" AS (
      WITH account_data_by_period AS (
        SELECT 
          user_id,
      ...
      fallback_data AS (
        SELECT 
          user_id,
          DATE_TRUNC('year', MIN(week_start::date)) as period_start,
          -- Total P&L since inception
          MAX(cumulative_portfolio_value::numeric) - MIN(cumulative_portfolio_value::numeric) - 
          (MAX(cumulative_transfers::numeric) - MIN(cumulative_transfers::numeric)) as period_pnl,
          -- Total percentage since inception
        ...
        FROM account_value_over_time
        GROUP BY user_id
        HAVING COUNT(DISTINCT DATE_TRUNC('year', week_start::date)) = 1  -- Only one period exists
      )
      SELECT 
        user_id,
        period_start,
