...
CREATE VIEW "public"."transaction_details" AS (select "dim_account"."user_id", "dim_security"."symbol", "dim_security"."option_type", "dim_security"."strike_price", JSON_AGG(
        JSON_BUILD_OBJECT(
...
CREATE VIEW "public"."position_calculations" AS (select "dim_account"."user_id", "dim_security"."symbol", "dim_security"."option_type", "dim_security"."strike_price", CONCAT("dim_security"."symbol", '-', COALESCE("dim_security"."option_type", 'STOCK'), '-', COALESCE("dim_security"."strike_price"::text, '0')) as "position_key", SUM(
      ...
      END) as "unrealized_pnl" from "fact_transactions" inner join "dim_account" on "fact_transactions"."account_key" = "dim_account"."account_key" inner join "dim_security" on "fact_transactions"."security_key" = "dim_security"."security_key" inner join "dim_transaction_type" on "fact_transactions"."transaction_type_key" = "dim_transaction_type"."transaction_type_key" inner join "dim_date" on "fact_transactions"."date_key" = "dim_date"."date_key" where "dim_transaction_type"."action_code" NOT IN ('dividend', 'interest', 'transfer', 'other') group by "dim_account"."user_id", "dim_security"."symbol", "dim_security"."option_type", "dim_security"."strike_price", "dim_security"."expiry_date");--> statement-breakpoint
