...
CREATE VIEW "public"."view_weekly_returns" AS (
  ...
  returns_data AS (
    SELECT 
      ...
      (((cumulative_portfolio_value - cumulative_transfers) - LAG(cumulative_portfolio_value - cumulative_transfers, 1) OVER (PARTITION BY user_id ORDER BY week_start)) / LAG(cumulative_portfolio_value - cumulative_transfers, 1) OVER (PARTITION BY user_id ORDER BY week_start) * 100) as weekly_return_percent
