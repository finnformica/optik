...
CREATE TABLE "dim_security" (
	...
	CONSTRAINT "dim_security_symbol_security_type_option_type_strike_price_expiry_date_pk" PRIMARY KEY("symbol","security_type","option_type","strike_price","expiry_date"),
...
CREATE TABLE "fact_current_positions" (
	...
	"security_symbol" varchar(50) NOT NULL,
	"security_type" varchar(20) NOT NULL,
	"security_option_type" varchar(10),
	"security_strike_price" numeric(18, 8),
	"security_expiry_date" date,
	...
	CONSTRAINT "fact_current_positions_account_key_security_symbol_security_type_security_option_type_security_strike_price_security_expiry_date_pk" PRIMARY KEY("account_key","security_symbol","security_type","security_option_type","security_strike_price","security_expiry_date")
);
--> statement-breakpoint
CREATE TABLE "fact_transactions" (
	...
	"broker_transaction_id" varchar(50),
	"order_id" varchar(50),
...
ALTER TABLE "fact_current_positions" ADD CONSTRAINT "fk_fact_current_positions_security" FOREIGN KEY ("security_symbol","security_type","security_option_type","security_strike_price","security_expiry_date") REFERENCES "public"."dim_security"("symbol","security_type","option_type","strike_price","expiry_date") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "fact_transactions" ADD CONSTRAINT "fk_fact_transactions_security" FOREIGN KEY ("security_symbol","security_type","security_option_type","security_strike_price","security_expiry_date") REFERENCES "public"."dim_security"("symbol","security_type","option_type","strike_price","expiry_date") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_fact_positions_underlying" ON "fact_current_positions" USING btree ("security_symbol");--> statement-breakpoint
...
CREATE INDEX "idx_fact_transactions_security" ON "fact_transactions" USING btree ("security_symbol","security_type");--> statement-breakpoint
...
CREATE VIEW "public"."view_company_positions" AS (
  ...
  JOIN dim_security s ON (
    p.security_symbol = s.symbol AND
    p.security_type = s.security_type AND
    COALESCE(p.security_option_type, '') = COALESCE(s.option_type, '') AND
    COALESCE(p.security_strike_price, 0) = COALESCE(s.strike_price, 0) AND
    COALESCE(p.security_expiry_date, '1900-01-01') = COALESCE(s.expiry_date, '1900-01-01')
  )
...
CREATE VIEW "public"."view_current_positions" AS (
  SELECT 
    a.user_id,
    s.symbol,
    s.security_type,
    ...
    p.cost_basis,
    p.average_price,
    DATE_PART('day', COALESCE(s.expiry_date, CURRENT_DATE + INTERVAL '1000 days') - CURRENT_DATE)::integer as days_to_expiry,
    CASE WHEN p.quantity_held > 0 THEN 'LONG' ELSE 'SHORT' END as direction
  ...
  JOIN dim_account a ON p.account_key = a.account_key
  WHERE p.quantity_held != 0
  ORDER BY ABS(p.cost_basis) DESC
);--> statement-breakpoint
CREATE VIEW "public"."view_portfolio_distribution" AS (
  ...
  FROM fact_current_positions p
  JOIN dim_security s ON (
    p.security_symbol = s.symbol AND p.security_type = s.security_type AND
    COALESCE(p.security_option_type, '') = COALESCE(s.option_type, '') AND
    COALESCE(p.security_strike_price, 0) = COALESCE(s.strike_price, 0) AND
    COALESCE(p.security_expiry_date, '1900-01-01') = COALESCE(s.expiry_date, '1900-01-01')
  )
