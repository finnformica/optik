...
CREATE VIEW "public"."view_daily_activity" WITH (security_invoker = true) AS (
  WITH expiry_range AS (
    ...
    FROM "dim_account" a
    LEFT JOIN "view_position" vp ON a.account_key = vp.account_key
    LEFT JOIN "dim_security" s ON s.symbol = vp.symbol AND s.expiry_date IS NOT NULL AND s.expiry_date >= CURRENT_DATE AND ABS(CAST(vp.quantity_held AS DECIMAL)) > 0
