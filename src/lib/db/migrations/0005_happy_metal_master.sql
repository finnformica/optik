DROP VIEW IF EXISTS "public"."position_calculations" CASCADE;--> statement-breakpoint
CREATE OR REPLACE VIEW "public"."position_calculations" AS (select "dim_account"."user_id", "fact_transactions"."security_key", "fact_transactions"."account_key", SUM(
        CASE WHEN "dim_transaction_type"."action_category" = 'TRADE' 
        THEN "fact_transactions"."quantity" * "dim_transaction_type"."direction"
        ELSE 0 END
      ) as "net_quantity", SUM(
        CASE WHEN "dim_transaction_type"."action_category" = 'TRADE' AND "dim_transaction_type"."direction" = -1
        THEN ABS("fact_transactions"."gross_amount")  -- Money going out (buys)
        ELSE 0 END
      ) as "cost_basis", SUM(
        CASE WHEN "dim_transaction_type"."action_category" = 'TRADE' AND "dim_transaction_type"."direction" = 1
