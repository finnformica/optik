...
CREATE VIEW "public"."position_calculations" AS (select "dim_account"."user_id", "fact_transactions"."security_key", "fact_transactions"."account_key", SUM(
        ...
        CASE WHEN "dim_transaction_type"."action_category" = 'TRADE'
        THEN ABS("fact_transactions"."gross_amount")  -- Total money involved in trades
        ELSE 0 END
      ) as "cost_basis", SUM(
...
CREATE VIEW "public"."view_positions" AS (
  SELECT 
    ...
    -- Position status (CLOSED if expired or zero quantity)
    CASE 
      WHEN ds.expiry_date IS NOT NULL AND ds.expiry_date < CURRENT_DATE THEN 'CLOSED'
