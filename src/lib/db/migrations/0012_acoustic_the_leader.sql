...
CREATE VIEW "public"."view_weekly_returns" AS (
  ...
  returns_data AS (
    SELECT 
      ...
      -- Calculate weekly return percentage (gains / starting portfolio value * 100)
      CASE 
        WHEN LAG(cumulative_portfolio_value - cumulative_transfers, 1) OVER (PARTITION BY user_id ORDER BY week_start) > 0
        THEN (weekly_gains / LAG(cumulative_portfolio_value - cumulative_transfers, 1) OVER (PARTITION BY user_id ORDER BY week_start) * 100)
        ELSE 0
