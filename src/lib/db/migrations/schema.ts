import { pgTable, unique, serial, varchar, text, timestamp, foreignKey, integer, boolean, index, check, numeric, date, json, pgView } from "drizzle-orm/pg-core"
...
export const dimSecurity = pgTable("dim_security", {
	...
	index("idx_dim_security_symbol").using("btree", table.symbol.asc().nullsLast().op("text_ops")),
	index("idx_dim_security_type").using("btree", table.securityType.asc().nullsLast().op("text_ops")),
	index("idx_dim_security_underlying").using("btree", table.underlyingSymbol.asc().nullsLast().op("text_ops")),
	check("check_stock_option", sql`(((security_type)::text = 'STOCK'::text) AND ((underlying_symbol)::text = (symbol)::text) AND (option_type IS NULL) AND (strike_price IS NULL) AND (expiry_date IS NULL)) OR (((security_type)::text = 'OPTION'::text) AND ((underlying_symbol)::text <> (symbol)::text) AND (option_type IS NOT NULL) AND (strike_price IS NOT NULL) AND (expiry_date IS NOT NULL))`),
]);
...
export const dimDate = pgTable("dim_date", {
	...
	index("idx_dim_date_full_date").using("btree", table.fullDate.asc().nullsLast().op("date_ops")),
	index("idx_dim_date_year_month").using("btree", table.year.asc().nullsLast().op("int4_ops"), table.monthNumber.asc().nullsLast().op("int4_ops")),
...
export const dimBroker = pgTable("dim_broker", {
	...
	unique("dim_broker_broker_code_unique").on(table.brokerCode),
]);
...
export const rawTransactions = pgTable("raw_transactions", {
	...
	brokerTimestamp: timestamp("broker_timestamp", { mode: 'string' }).notNull(),
	processedAt: timestamp("processed_at", { mode: 'string' }),
	createdAt: timestamp("created_at", { mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_raw_transactions_broker_id").using("btree", table.brokerTransactionId.asc().nullsLast().op("text_ops")),
	index("idx_raw_transactions_user_status").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
			name: "raw_transactions_user_id_users_id_fk"
		}),
	unique("unique_broker_transaction").on(table.brokerCode, table.brokerTransactionId),
]);
...
export const factTransactions = pgTable("fact_transactions", {
	...
	brokerTransactionId: varchar("broker_transaction_id", { length: 50 }).primaryKey().notNull(),
	...
	index("idx_fact_transactions_account").using("btree", table.accountKey.asc().nullsLast().op("int4_ops")),
	index("idx_fact_transactions_date").using("btree", table.dateKey.asc().nullsLast().op("int4_ops")),
	index("idx_fact_transactions_security").using("btree", table.securityKey.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const dimBrokerAccounts = pgTable("dim_broker_accounts", {
	...
	index("idx_broker_accounts_account_broker").using("btree", table.accountKey.asc().nullsLast().op("int4_ops"), table.brokerKey.asc().nullsLast().op("int4_ops")),
	index("idx_broker_accounts_hash").using("btree", table.brokerAccountHash.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.accountKey],
			foreignColumns: [dimAccount.accountKey],
...
export const viewWeeklyReturns = pgView("view_weekly_returns", {	userId: integer("user_id"),
...
}).as(sql`SELECT a.user_id, s.symbol, s.underlying_symbol, max(s.security_type::text) AS security_type, max(s.option_type::text) AS option_type, s.strike_price, sum(ft.quantity) AS quantity_held, sum( CASE WHEN s.security_type::text = 'STOCK'::text THEN abs(ft.gross_amount) WHEN s.security_type::text = 'OPTION'::text THEN abs(ft.quantity) * s.strike_price * 100::numeric ELSE abs(ft.gross_amount) END) AS position_value, sum( CASE WHEN s.security_type::text = 'STOCK'::text THEN ft.net_amount WHEN s.security_type::text = 'OPTION'::text THEN abs(ft.quantity) * s.strike_price * 100::numeric - ft.fees ELSE ft.net_amount END) AS net_position_value, sum(abs(ft.quantity * ft.price_per_unit)) / NULLIF(sum(abs(ft.quantity)), 0::numeric) AS average_price, sum(ft.fees) AS total_fees, CASE WHEN max(s.expiry_date) IS NOT NULL THEN max(s.expiry_date) - CURRENT_DATE ELSE NULL::integer END AS days_to_expiry, CASE WHEN max(s.security_type::text) = 'OPTION'::text AND abs(sum(ft.quantity)) > 0.001 THEN 'OPEN'::text WHEN max(s.security_type::text) = 'STOCK'::text AND abs(sum(ft.quantity)) > 0.0001 THEN 'OPEN'::text ELSE 'CLOSED'::text END AS position_status, min(d.full_date) AS first_transaction_date, max(d.full_date) AS last_transaction_date, count(*)::integer AS transaction_count FROM fact_transactions ft JOIN dim_security s ON ft.security_key = s.security_key JOIN dim_account a ON ft.account_key = a.account_key JOIN dim_transaction_type tt ON ft.transaction_type_key = tt.transaction_type_key JOIN dim_date d ON ft.date_key = d.date_key WHERE tt.action_category::text = ANY (ARRAY['TRADE'::character varying, 'CORPORATE'::character varying]::text[]) GROUP BY a.user_id, s.symbol, s.underlying_symbol, s.strike_price`);

export const viewPortfolioSummary = pgView("view_portfolio_summary", {	userId: integer("user_id"),
