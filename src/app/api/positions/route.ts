...
export async function GET(request: NextRequest) {
  try {
    ...
    const transformPositionsData = (data: PositionsBySymbol[]) => {
      return data
        .filter(row => row.symbol) // Filter out null tickers
        .map(row => {
          ...
          // Apply symbol filter and ensure group has positions
          const matchesSymbol = !symbol || group.symbol?.toLowerCase().includes(symbol.toLowerCase());
          return matchesSymbol && group.positions.length > 0;
        });
    };
